...
With no probability option, the PC validator runs in `AUTO: DEVICE CUT-IN OUTPUT` mode. It uses the embedded front/corner model thresholds, history filters, controller output, and final deduplication instead of one review threshold, and pauses only when the production path emits a CUT-IN. Raw trajectory `IN`/`OUT` values remain visible, but a shadow probability alone cannot pause playback. Use `radar_lead_validation_review.py --prob 0.70` only for manual raw-probability review; changing that slider saves the value, and `--manual-prob` reuses the last saved manual value. Manual `PROB` changes PC highlighting and pauses only, never the on-device threshold.
