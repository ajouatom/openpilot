...
This setting applies only when `CarrotRadarMode=1`. It jointly adjusts the CUT-IN probability threshold, measured directional-consistency threshold, and confirmation dwell. Level `3` exactly preserves the current corner/front probability thresholds of `0.30/0.67`, normal `0.25 s` confirmation, and front directional-consistency threshold of `0.75`. Higher levels accept lower probability and directional consistency with a shorter confirmation; lower levels do the reverse. Physical requirements such as sustained future corridor overlap, range limits, being closer than leadOne, and stationary-noise rejection remain active at every sensitivity. At `0`, leadOne and radar-state publication continue, but a new physical CUT-IN is not selected as leadTwo. This setting does not change conventional `radard` or `EnableCornerRadar`.
