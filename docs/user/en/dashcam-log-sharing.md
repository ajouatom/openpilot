...
When the Carrot upload job finishes, the device sends the same result report directly to the configured Discord webhook. A Discord notification failure does not undo an already completed log upload; if no notification appears, post the completion screen's `Copy` result to the channel manually. Toss uploads remain exclusive to Toss and never send this direct Discord report.
