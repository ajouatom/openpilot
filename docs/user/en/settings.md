...
`ClusterHudScreenMode=5` shows a live driving report in the right-hand panel. It summarizes driving time, distance, average and maximum speed, the automated-driving ratio, hard acceleration/braking/corner counts, and CPU, temperature, memory, and disk usage. The lower-right corner shows the device pitch and yaw from the stored calibration. The trace does not use a map or route server: it is calculated from `livePose`, steering angle, and vehicle speed. When GPS is available, its bearing aligns the `livePose` frame and each position correction translates the existing trace by the same amount, preventing a correction from creating a false bend. Point count and range are bounded, with a maximum displayed radius of about 30 km.
