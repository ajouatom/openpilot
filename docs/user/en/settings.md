...
`LeadAccelResponse` adjusts how quickly the vehicle follows a lead starting or accelerating at following-distance level 1. Level 1 is weak, level 2 is mild, level 3 is the brisk everyday choice, level 4 urgently recovers an opening gap, and test level 5 approaches the final acceleration envelope quickly. When a stable radar lead accelerates but MPC changes to a `cruise` source, levels 3/4/5 use gap error and opening relative speed and may use up to 65/95/100% of the final `CruiseMaxVals` envelope. Normal control resumes immediately when lead acceleration or set-speed headroom ends. See [Lead-vehicle response](cruise-gap.md#lead-response) for its activation gates and per-level limits.
