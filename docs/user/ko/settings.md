...
`ClusterHudScreenMode=5`는 오른쪽 패널에 실시간 주행 리포트를 표시합니다. 주행 시간·거리·평균/최고 속도·자동주행 비율·급가감속/급코너 횟수와 CPU·온도·메모리·디스크 사용량을 요약하며, 아래 오른쪽에는 저장된 캘리브레이션의 디바이스 피치·요 각도를 표시합니다. 궤적은 지도나 경로 서버를 사용하지 않고 `livePose`, 조향각과 차속으로 계산합니다. GPS가 수신되면 진행방향에 맞춰 `livePose` 좌표계를 정렬하고, 위치 보정은 기존 궤적 전체에 같은 변위를 적용해 보정 순간에 거짓 굴곡이 생기지 않도록 합니다. 표시 점 수와 반경은 제한되며 최대 반경은 약 30 km입니다.
