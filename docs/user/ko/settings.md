...
| 레이더 | `EnableRadarTracks`, `EnableCornerRadar`, `RadarDPathMode` | SCC 레이더, 레이더 트랙, 코너 레이더와 dPath leadTwo 처리 |
...
> [!CAUTION]
> `HyundaiCameraSCC`, `CanfdHDA2`, `EnableRadarTracks`, `RadarDPathMode`, `SpeedFromPCM`은 잘못 설정하면 차량 인식, SCC, 레이더와 가감속 동작이 달라질 수 있습니다. 차종, 연식, HDA 구성, 하네스 연결 위치와 순정 ACC 사용 여부를 확인한 뒤 변경하세요.
...
- `EnableRadarTracks`, `EnableCornerRadar`, `RadarDPathMode`
