...
`ClusterHudScreenMode=5`는 오른쪽 패널에 실시간 주행 리포트를 표시합니다. 기본 화면 모드(`0`)에서도 실시간 내비가 수신되지 않는 동안에는 이 리포트가 자동으로 표시되고, 수신이 시작되면 내비 패널로 돌아갑니다. 주행 시간·거리·평균/최고 속도·자동주행 비율·급가감속/급코너 횟수와 CPU·온도·메모리·디스크 사용량을 요약하며, 아래 오른쪽에는 저장된 캘리브레이션의 디바이스 피치·요 각도를 표시합니다. 왼쪽 카메라 영역 아래에는 브랜치·네트워크 주소·프레임률 상태를 계속 표시하고, 리포트와 겹치는 화면 오른쪽 아래의 코어 사용량 문구만 생략합니다. 궤적은 지도나 경로 서버를 사용하지 않고 `livePose`, 조향각과 차속으로 계산합니다. GPS가 수신되면 진행방향에 맞춰 기존 궤적 전체를 회전해 화면 위쪽을 북쪽으로 정렬하고, 위치 보정은 기존 궤적 전체에 같은 변위를 적용해 보정 순간에 거짓 굴곡이 생기지 않도록 합니다. 표시 반경은 250 m에서 시작해 목표값이 10 m 단위로 늘어나고 렌더러가 그 값까지 부드럽게 확대하며, 최대 1 km와 1 km 밖의 이어진 과거 경로 제외 기준은 유지합니다.
