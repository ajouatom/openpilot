...
설정 항목의 값 조절 부분을 제외한 본문을 누르면 해당 항목의 상세 화면으로 이동합니다. 상세 화면의 **상세 설명**은 장치에 함께 설치된 `docs/user/ko` Markdown 원문에서 현재 항목이 속한 구역만 읽어 표시합니다.

- 인터넷이나 GitHub 연결 없이 장치 내부 문서를 사용합니다.
- **상세 설명** 제목을 눌러 내용을 접거나 펼칠 수 있습니다.
- Markdown을 읽지 못해도 설정값 확인·변경 기능은 계속 사용할 수 있습니다.
- Carrot Web이 한국어이면 한국어 원문, 영어이면 영어 원문을 표시합니다.
- 중국어 상세 원문이 아직 없으면 중국어 제목·짧은 설명은 유지하고 상세 설명만 영어로 표시합니다.

같은 세부 구역의 여러 파라미터는 공통 설명을 공유할 수 있습니다. 화면의 짧은 설명과 상세 설명이 다르면 현재 브랜치의 코드와 함께 관리되는 이 원문을 기준으로 문제를 확인하세요.
