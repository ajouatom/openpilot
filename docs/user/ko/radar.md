...
하이브리드 방식은 전방 레이더 단독 끼어들기 후보의 횡이동이 여러 이력 구간에서 일관될 때만 확정합니다. 전방·코너 레이더가 함께 확인한 작은 표적은 비전 횡위치가 짧게 흔들려도 기존 매칭을 유지하여 선행차 끊김을 줄입니다.
...
openpilot이 활성화된 상태에서 새로운 끼어들기 차량이 확정되면 높은 2단계 알림음이 재생됩니다. 자차가 약 14 km/h보다 빠르게 주행할 때 새로운 정지 레이더 선행차가 선택되면 더 낮은 2단계 알림음이 재생됩니다. 같은 객체가 계속 추적되는 동안에는 한 번만 울리며, 짧은 레이더 끊김만으로 즉시 다시 울리지 않습니다. 스피커가 없는 C3X Lite에서는 같은 이벤트를 서로 다른 GPIO 부저 패턴으로 알립니다.

정지차 알림은 속도가 3 km/h 미만인 레이더 확정 리드에만 적용되며 비전 단독 리드에는 적용되지 않습니다. 이 알림음은 선택된 레이더 결과를 알려줄 뿐 선행차 선택이나 종방향 제어를 변경하지 않습니다. 더 높은 우선순위의 안전 경고가 발생하면 해당 경고음이 우선할 수 있습니다.
