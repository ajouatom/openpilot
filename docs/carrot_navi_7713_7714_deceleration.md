...
해석한다. A-CAN `0x380` byte 3의 bit 6이 확인된 카메라는 해당 비트가 꺼지면 종료하며,
`MapSource=2`가 남아 있어도 다시 활성화하지 않는다. 두 상태가 모두 종료된 뒤 다음 단속을
받는다. 별도 비트가 한 번도 확인되지 않고 byte 3이 0인 경우에는 `MapSource=2`를 현재
단속으로 사용한다. 이는 2026-09-21 PV5 후면단속 로그에서 확인된 누락을 보완하며,
다른 미확인 상태값이나 통과 후 값 `0x04`에는 대체 판정을 적용하지 않는다. 두 메시지는
프레임 길이·버스 상태와 1초 이내 수신을 확인하며, 유실 중에도 기존 통과 판정은 보존한다.
가까운 거리 프로파일이 없으면 현재 안내의 가상거리로 감속하고 먼 미래 카메라와 분리한다.
기록 재생에서 문제 시점의 상한은 약 135에서 50 km/h로 바뀌고 안내 종료 때 해제되었다.
이는 기록 재생 검증이며 수정 후 실제 차량의 통과 시점 검증은 아직 아니다.
PV5에서는 일반 카메라와 Value 6
