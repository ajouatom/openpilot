...
`0x364 MapSource=2`와 가까운 거리 프로파일을 연결한다. 연결된 현재 카메라는 알림 종료나
다른 제한속도 안내에도 남은 거리까지 보존하며, 다음 카메라는 대기 목록에 둔다.
휠 주행거리로 현재 목표에 도달하면 해당 이벤트를 제거하고 다음 대상으로 전환한다.
완료된 카메라와 같은 속도의 안내가 남아 있어도 가상거리를 재시작하거나 다음 카메라를
현재 안내에 다시 연결하지 않는다. 안내 종료 또는 제한속도 변경 후 새 안내를 받는다.
경로 재설정과 설정 해제에 따른 기존 목록 무효화는 유지한다.

가까운 거리 프로파일이 없으면 단속 안내의 가상거리로 처리한다. 이 경우 거리 0은
실제 통과 증거가 아니므로 안내가 끝날 때 해제한다. 가상거리 안내는 두 메시지의
프레임 길이·버스 상태·1초 이내 수신을 확인한다. 유효하게 받은 실제 거리 프로파일은
휠 주행거리로 계속 추적하며, 신호 유실이 곧 해당 위치의 통과를 뜻하지는 않는다.
PV5에서는 일반 카메라와 Value 6 방지턱을 지원하고, 종류 7 프로파일만으로 구간단속·
30 km/h 구간을 활성화하지 않는다.

2026-09-21 후속 로그는 알림 종료 때 남은 거리 139m의 30 km/h 카메라가 삭제되고
1327m 앞 50 km/h 카메라로 바뀌는 오류를 재현했다. 수정 재생은 현재 카메라를 유지한다.
이전 구간이 없어 초기 대기 목록의 목표는 기록된 거리와 휠 주행거리에서 복원했으며,
기존 코드 재생의 거리 최대 오차는 0.000066m였다. 로그 종료에도 현재 카메라가 남아 있어
거리 소진 후 전환은 별도 합성 테스트로 검증했다. 앞선 6구간의 거리 없는 50 km/h 안내도
재생 확인했다. 어느 검사도 수정 후 실제 차량의 통과 위치 검증을 대신하지 않는다.
