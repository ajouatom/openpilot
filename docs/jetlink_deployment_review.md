...
계기판 렌더러의 배지 이미지 확인을 수행했다. 2026-09-25에는 기존 C4와 Jetson에
`f5749653f5`를 배포했다. C4의 활성 배지와 Jetson으로 전달된 계기판 스냅샷에서
`jetSON`을 확인했고, USB H.264 출력은 10 Hz, 전달 서비스는 26/26 alive였다.
이는 실제 패널을 촬영한 시각 검증이나 Mac 실물 검증을 대신하지 않는다.

호스트 번들은 별도 디렉터리에 준비하고 기존 소스를 복구용으로 보존한 뒤
정차·비개입 상태에서 서비스를 전환했다. Windows 체크섬 파일의 CRLF 때문에
Linux `sha256sum`이 파일명 끝의 CR까지 읽는 오류를 발견해 LF로 수정했으며,
새 번들은 Jetson의 실제 해시 검증을 통과했다. 이것은 준비된 기존 장치의
소스 업데이트 검증이다. 빈 Jetson의 최초 설치, 자동 복구 및 Mac 실물 확인은
여전히 별도 검증이 필요하다. 배포 후 타이밍 결과는
[실험 기록](jetlink_experiment.md#two-sided-usb-timing-investigation-2026-09-25)에 남겼다.
