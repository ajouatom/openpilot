...
def _env_secret(name: str) -> str:
  ...
  secret_path = os.environ.get(f"{name}_FILE", "").strip()
  if not secret_path:
    return ""

  try:
    for raw_line in Path(secret_path).read_text(encoding="utf-8").splitlines():
      line = raw_line.strip()
...
class Config:
  ...
  def from_env(cls) -> Config:
    ...
    return cls(
      ...
      route_admin_key=_env_secret("CARROT_ROUTE_ADMIN_KEY"),
