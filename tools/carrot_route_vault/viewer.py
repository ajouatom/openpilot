...
PUBLIC_SELECTION_RE = re.compile(
  r"^(?P<route>[A-Za-z0-9_.|-]{20})--(?P<slice>\d+(?::\d+(?::\d+)?)?)$",
)
...
def _public_selection(value: Any) -> tuple[str, str, frozenset[int]]:
  text = str(value or "").strip()
  match = PUBLIC_SELECTION_RE.fullmatch(text)
  if not match:
    raise web.HTTPBadRequest(text="invalid public route selection")
  route = match.group("route")
  selection = match.group("slice")
  parts = selection.split(":")
  try:
    if len(parts) == 1:
      indexes = range(int(parts[0]), int(parts[0]) + 1)
    ...
    raise web.HTTPBadRequest(text="invalid public segment slice") from exc
  selected = frozenset(indexes)
  if not selected or min(selected) < 0 or max(selected) > 1000000 or len(selected) > 1000:
    raise web.HTTPBadRequest(text="invalid public segment slice")
  return route, selection, selected


def _is_private_route_file(filename: str) -> bool:
  # The driver-facing camera can contain identifiable faces. Keep every
  # dcamera variant private even if a new extension is added later.
  return filename.casefold().startswith("dcamera")
...
class RouteViewer:
  def __init__(
    ...
    *,
    ...
    self.radar_jobs = RadarJobs(self.db_path.parent / "route_radar_cache")
    self._init_db()
  ...
  def _file_path(self, directory: Any, route: Any, index: Any, filename: Any) -> Path:
    ...
    if _is_private_route_file(name) or name not in VIEWABLE_FILES:
  ...
  def _model_path(self, family: Any, filename: Any) -> Path:
    family_name = str(family or "").strip()
    file_name = str(filename or "").strip()
    if not MODEL_COMPONENT_RE.fullmatch(family_name) or file_name not in MODEL_CONTENT_TYPES:
      raise web.HTTPNotFound(text="model file not found")

    root = self.models_root.resolve()
    family_path = root / family_name
    path = family_path / file_name
    try:
      if family_path.is_symlink() or not family_path.is_dir() or path.is_symlink() or not path.is_file():
        raise web.HTTPNotFound(text="model file not found")
      resolved = path.resolve()
    except OSError as exc:
      raise web.HTTPNotFound(text="model file not found") from exc
    ...
    for directory_path in root.iterdir():
      try:
        ...
        for segment_path in directory_path.iterdir():
          ...
          if not match:
            continue
          grouped[match.group("route")].append(
            (int(match.group("index")), segment_path.stat().st_mtime, segment_path.name),
          )
        ...
        for route, segments in grouped.items():
          ...
          routes.append({
            ...
            "segments": [
              {"name": item[2], "index": item[0], "modifiedEpoch": int(item[1])}
              for item in ordered
            ],
          })
      except OSError:
  ...
  def _manifest_sync(
    ...
    allowed_indexes: frozenset[int] | None = None,
  ) -> dict[str, Any]:
    ...
    for segment_path in directory_path.glob(f"{route}--*"):
      try:
        ...
        for path in sorted(segment_path.iterdir(), key=lambda item: item.name):
          if _is_private_route_file(path.name) or path.name not in VIEWABLE_FILES or path.is_symlink() or not path.is_file():
            continue
          stat = path.stat()
  ...
  @staticmethod
  def _rlog_target(api_host: str, device_id: str, route: str, segments: list[dict[str, Any]]) -> str:
    rlog_names = {"rlog", "rlog.zst", "rlog.bz2"}
    indexes = sorted({
      int(segment["index"])
      for segment in segments
      if any(str(item.get("name") or "") in rlog_names for item in segment.get("files", []))
    })
    if not api_host or not device_id or not indexes:
      return ""
    if len(indexes) == 1:
      segment_slice = str(indexes[0])
    else:
      step = indexes[1] - indexes[0]
      if step <= 0 or any(right - left != step for left, right in zip(indexes, indexes[1:], strict=False)):
        return ""
      segment_slice = f"{indexes[0]}:{indexes[-1] + step}"
      if step != 1:
        segment_slice += f":{step}"
    target = f"{device_id}/{route}/{segment_slice}/r"
    return target

  @classmethod
  def _cabana_command(cls, api_host: str, device_id: str, route: str, segments: list[dict[str, Any]]) -> str:
    target = cls._rlog_target(api_host, device_id, route, segments)
    cabana_target = target[:-2] if target.endswith("/r") else target
    return f'API_HOST="{api_host}" ./cabana "{cabana_target}"' if cabana_target else ""

  @classmethod
  def _plotjuggler_command(cls, api_host: str, device_id: str, route: str, segments: list[dict[str, Any]]) -> str:
    target = cls._rlog_target(api_host, device_id, route, segments)
    return f'API_HOST="{api_host}" python3 -m openpilot.tools.plotjuggler.juggle "{target}"' if target else ""

  @classmethod
  def _jotpluggler_command(cls, api_host: str, device_id: str, route: str, segments: list[dict[str, Any]]) -> str:
    target = cls._rlog_target(api_host, device_id, route, segments)
    return f'API_HOST="{api_host}" ./openpilot/tools/jotpluggler/jotpluggler "{target}"' if target else ""
  ...
  @staticmethod
  def _public_request_selection(request: web.Request) -> tuple[str, str, str, frozenset[int]]:
    directory = _component(request.match_info.get("directory"))
    route, slice_text, indexes = _public_selection(request.match_info.get("selection"))
    return directory, route, f"{route}--{slice_text}", indexes

  def _public_page_base(self, request: web.Request, directory: str, selection: str) -> str:
    return (
      f"{self._public_base(request)}/routes/" +
      f"{quote(directory, safe='')}/{quote(selection, safe=':|.-')}"
    )
  ...
  def _public_file_url(
    ...
    prefix = f"{self._public_page_base(request, directory, selection)}/files"
    return f"{prefix}/{quote(device, safe='')}/{quote(route, safe='|-')}/{index}/{quote(filename, safe='.-')}"

  async def _public_manifest(self, request: web.Request, *, include_video: bool = True) -> dict[str, Any]:
    directory, route, selection, indexes = self._public_request_selection(request)
    base = self._public_page_base(request, directory, selection)

    def file_url(device: str, index: int, filename: str) -> str:
      return self._public_file_url(request, directory, selection, device, route, index, filename)

    def video_url(index: int) -> str:
      return f"{base}/video/{index}.mp4" if include_video else ""
    ...
    actual_indexes = frozenset(int(segment["index"]) for segment in manifest["segments"])
    if actual_indexes != indexes:
      raise web.HTTPNotFound(text="one or more public route segments were not found")
    manifest["selection"] = selection
    manifest["publicUrl"] = base
    manifest["apiHost"] = f"{base}/api"
    device_id = str(manifest["deviceId"])
    manifest["cabanaCommand"] = self._cabana_command(
      manifest["apiHost"], device_id, route, manifest["segments"],
    )
    manifest["plotJugglerCommand"] = self._plotjuggler_command(
      manifest["apiHost"], device_id, route, manifest["segments"],
    )
    manifest["jotPlugglerCommand"] = self._jotpluggler_command(
      manifest["apiHost"], device_id, route, manifest["segments"],
    )
    if include_video:
      for segment in manifest["segments"]:
        if segment["videoUrl"]:
          segment["previewUrl"] = f"{base}/preview/{segment['index']}.jpg"
    return manifest

  async def _public_for_route(self, request: web.Request, canonical_route: str) -> dict[str, Any]:
    manifest = await self._public_manifest(request, include_video=False)
    value = str(canonical_route or "").replace("/", "|")
    if value != str(manifest["canonicalRoute"]):
      raise web.HTTPNotFound(text="route not found")
    return manifest
  ...
  async def api_admin_route(self, request: web.Request) -> web.Response:
    ...
    preview_prefix = f"{base}/admin/preview/{quote(directory, safe='')}/{quote(route, safe='|-')}"
    for segment in manifest["segments"]:
      if segment["videoUrl"]:
        segment["previewUrl"] = f"{preview_prefix}/{segment['index']}.jpg"
  ...
  async def api_admin_share_create(self, request: web.Request) -> web.Response:
    ...
    return web.json_response({
      ...
      "cabanaCommand": self._cabana_command(api_host, device_id, route, manifest["segments"]),
      "plotJugglerCommand": self._plotjuggler_command(api_host, device_id, route, manifest["segments"]),
      "jotPlugglerCommand": self._jotpluggler_command(api_host, device_id, route, manifest["segments"]),
    })
  ...
  async def api_share_manifest(self, request: web.Request) -> web.Response:
    ...
    manifest = await asyncio.to_thread(self._manifest_sync, directory, route, file_url, video_url)
    preview_prefix = f"{base}/s/{quote(token, safe='._-')}/preview"
    for segment in manifest["segments"]:
      if segment["videoUrl"]:
        segment["previewUrl"] = f"{preview_prefix}/{segment['index']}.jpg"
  ...
  async def api_public_manifest(self, request: web.Request) -> web.Response:
    return web.json_response(await self._public_manifest(request))

  async def api_public_route_files(self, request: web.Request) -> web.Response:
    manifest = await self._public_for_route(request, request.match_info.get("canonical", ""))
    groups: dict[str, list[str]] = {name: [] for name in FILE_GROUPS.values()}
    for segment in manifest["segments"]:
      for item in segment["files"]:
        group = FILE_GROUPS.get(item["name"])
  ...
  async def api_public_route_metadata(self, request: web.Request) -> web.Response:
    manifest = await self._public_for_route(request, request.match_info.get("canonical", ""))
    return web.json_response({
      "fullname": manifest["canonicalRoute"],
      "maxqlog": int(manifest["lastSegment"]),
      "maxrlog": int(manifest["lastSegment"]),
      "segmentCount": int(manifest["segmentCount"]),
    })

  async def api_public_devices(self, request: web.Request) -> web.Response:
    manifest = await self._public_manifest(request, include_video=False)
    device_id = str(manifest["deviceId"])
    return web.json_response([
      {"dongle_id": device_id, "alias": "Public upload", "device_type": "shared"},
    ] if device_id else [])

  async def api_public_device_routes(self, request: web.Request) -> web.Response:
    manifest = await self._public_manifest(request, include_video=False)
    device_id = str(manifest["deviceId"])
    if request.match_info.get("device", "").lower() != device_id:
      raise web.HTTPNotFound(text="device not found")
  ...
  async def model_file(self, request: web.Request) -> web.StreamResponse:
    filename = request.match_info.get("filename", "")
    path = await asyncio.to_thread(
      self._model_path,
      request.match_info.get("family"),
      filename,
    )
  ...
  async def public_file(self, request: web.Request) -> web.StreamResponse:
    ...
    expected_device = self._device_id(directory)
    requested_route = _component(request.match_info.get("route"), route=True)
    index = _segment_index(request.match_info.get("index"))
    if device != expected_device or requested_route != route or index not in indexes:
      raise web.HTTPNotFound(text="route file not found")
    filename = request.match_info.get("filename", "")
    path = await asyncio.to_thread(self._file_path, directory, route, index, filename)
    return web.FileResponse(path, headers={"Content-Type": CONTENT_TYPES.get(filename, "application/octet-stream")})

  async def public_video(self, request: web.Request) -> web.StreamResponse:
    directory, route, _selection, indexes = self._public_request_selection(request)
    index_text = str(request.match_info.get("index", ""))
    if index_text.endswith(".mp4"):
      index_text = index_text[:-4]
    index = _segment_index(index_text)
    if index not in indexes:
      raise web.HTTPNotFound(text="route video not found")
    return await self._video(directory, route, index)

  async def public_preview(self, request: web.Request) -> web.StreamResponse:
    directory, route, _selection, indexes = self._public_request_selection(request)
    index_text = str(request.match_info.get("index", ""))
    if index_text.endswith(".jpg"):
      index_text = index_text[:-4]
    index = _segment_index(index_text)
    if index not in indexes:
      raise web.HTTPNotFound(text="route preview not found")
    return await self._preview(directory, route, index)
  ...
  def _prune_video_cache_sync(self) -> None:
    ...
    total = 0
    for path in self.video_cache_root.iterdir():
      if path.suffix.casefold() not in {".mp4", ".jpg"} or path.is_symlink() or not path.is_file():
        continue
      try:
        stat = path.stat()
      except OSError:
  ...
  def _prepare_preview_sync(self, directory: str, route: str, index: int) -> Path:
    segment_path = self._segment_path(directory, route, index)
    sources = (segment_path / "qcamera.mp4", segment_path / "qcamera.ts")
    source = next((
      path for path in sources
      if path.is_file() and not path.is_symlink() and path.stat().st_size > 0
    ), None)
    if source is None:
      raise web.HTTPNotFound(text="qcamera preview not found")
    stat = source.stat()
    cache_key = hashlib.sha256(
      f"preview\0{directory}\0{route}\0{index}\0{stat.st_size}\0{stat.st_mtime_ns}".encode(),
    ).hexdigest()
    self.video_cache_root.mkdir(parents=True, exist_ok=True)
    output = self.video_cache_root / f"{cache_key}.jpg"
    if output.is_file() and output.stat().st_size > 0:
      os.utime(output, None)
      return output
    ffmpeg = shutil.which(self.ffmpeg_binary) if not os.path.isabs(self.ffmpeg_binary) else self.ffmpeg_binary
    if not ffmpeg or not os.path.isfile(ffmpeg):
      raise web.HTTPServiceUnavailable(text="ffmpeg is not available")
    temp = self.video_cache_root / f".{cache_key}.{secrets.token_hex(6)}.part.jpg"
    try:
      result = subprocess.run(
        [
          ffmpeg, "-hide_banner", "-loglevel", "error", "-y",
          "-ss", "1", "-i", str(source),
          "-frames:v", "1", "-vf", "scale=960:-2", "-q:v", "3", str(temp),
        ],
        capture_output=True,
        text=True,
        timeout=60,
      ...
      if result.returncode != 0 or not temp.is_file() or temp.stat().st_size <= 0:
        logging.warning("route preview generation failed: %s", (result.stderr or "unknown ffmpeg error")[-500:])
        raise web.HTTPServiceUnavailable(text="preview generation failed")
      os.replace(temp, output)
      self._prune_video_cache_sync()
      return output
    finally:
  ...
  async def _preview(self, directory: str, route: str, index: int) -> web.StreamResponse:
    key = f"preview\0{directory}\0{route}\0{index}"
    lock = self._video_locks.setdefault(key, asyncio.Lock())
    async with lock:
      path = await asyncio.to_thread(self._prepare_preview_sync, directory, route, index)
  ...
  async def share_preview(self, request: web.Request) -> web.StreamResponse:
    row = await self._share(request.match_info.get("token", ""))
    index_text = str(request.match_info.get("index", ""))
    if index_text.endswith(".jpg"):
      index_text = index_text[:-4]
    return await self._preview(str(row["directory"]), str(row["route"]), _segment_index(index_text))
  ...
  async def admin_preview(self, request: web.Request) -> web.StreamResponse:
    self._require_admin(request)
    index_text = str(request.match_info.get("index", ""))
    if index_text.endswith(".jpg"):
      index_text = index_text[:-4]
  ...
  async def public_page(self, request: web.Request) -> web.Response:
    manifest = await self._public_manifest(request, include_video=False)
    title = html.escape(str(manifest["selection"]))
    return _html_response(SHARE_HTML.replace("__ROUTE_TITLE__", title))

  async def _radar_response(self, request, directory, route, index):
    source = None
    for name in ("rlog.zst", "rlog.bz2", "rlog", "qlog.zst", "qlog.bz2", "qlog"):
      try:
        source = self._file_path(directory, route, index, name)
        break
    ...
    try:
      sensitivity = int(request.query.get("sensitivity", "3"))
    except ValueError:
      raise web.HTTPBadRequest(text="invalid radar sensitivity") from None
    return await self.radar_jobs.response(source, request.query.get("sensor", "auto"), sensitivity)

  async def public_radar(self, request):
    directory, route, _selection, indexes = self._public_request_selection(request)
    index = _segment_index(request.match_info.get("index"))
    if index not in indexes:
      raise web.HTTPNotFound(text="route radar not found")
    return await self._radar_response(request, directory, route, index)

  async def share_radar(self, request):
    row = await self._share(request.match_info.get("token", ""))
    return await self._radar_response(request, str(row["directory"]), str(row["route"]),
                                      _segment_index(request.match_info.get("index")))

  async def radar_asset(self, request):
    return web.FileResponse(Path(__file__).with_name("radar_view.js"), headers={"Content-Type": "text/javascript"})

  def register(self, app: web.Application) -> None:
    app.on_cleanup.append(self.radar_jobs.close)
    app.router.add_get("/assets/radar_view.js", self.radar_asset)
    app.router.add_get("/s/{token}/radar/{index}", self.share_radar)
    app.router.add_get("/routes/{directory}/{selection}/radar/{index}", self.public_radar)
    ...
    app.router.add_get("/admin/preview/{directory}/{route}/{index}", self.admin_preview)
    ...
    app.router.add_get("/s/{token}/preview/{index}", self.share_preview)
    ...
    app.router.add_get(
      "/routes/{directory}/{selection}/files/{device}/{route}/{index}/{filename}",
      self.public_file,
    )
    app.router.add_get("/routes/{directory}/{selection}/video/{index}", self.public_video)
    app.router.add_get("/routes/{directory}/{selection}/preview/{index}", self.public_preview)
    for prefix in (
      "/routes/{directory}/{selection}/api/v1",
      "/routes/{directory}/{selection}/api//v1",
    ):
      app.router.add_get(f"{prefix}/route/{{canonical}}/files", self.api_public_route_files)
      app.router.add_get(f"{prefix}/route/{{canonical}}", self.api_public_route_metadata)
      app.router.add_get(f"{prefix}/me/devices/", self.api_public_devices)
      app.router.add_get(f"{prefix}/devices/{{device}}/routes_segments", self.api_public_device_routes)
      app.router.add_get(f"{prefix}/devices/{{device}}/routes/preserved", self.api_public_device_routes)
...
COMMON_STYLE = """
  ...
  *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;min-height:100vh;background:radial-gradient(circle at 8% -10%,rgba(255,138,54,.13),transparent 30rem),radial-gradient(circle at 92% 0,rgba(134,217,147,.07),transparent 24rem),var(--bg);line-height:1.5}button,input,select{font:inherit}button,a{touch-action:manipulation}button{cursor:pointer}a{color:inherit}.hidden{display:none!important}.grow{flex:1}.row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}.stack{display:grid;gap:12px}.muted{color:var(--muted)}.tiny{font-size:12px}.error{color:var(--red)}.success{color:var(--green)}
  ...
  .vault-tree{display:grid;gap:13px}.device-node,.drive-node{overflow:hidden;box-shadow:none}.device-node>summary,.drive-node>summary{list-style:none;cursor:pointer;display:flex;align-items:center;gap:13px}.device-node>summary::-webkit-details-marker,.drive-node>summary::-webkit-details-marker{display:none}.device-node>summary:before,.drive-node>summary:before{content:"›";color:var(--orange2);font-size:25px;line-height:1;transition:transform .16s}.device-node[open]>summary:before,.drive-node[open]>summary:before{transform:rotate(90deg)}.device-summary{padding:20px}.device-icon{width:42px;height:42px;border-radius:13px;background:#202b2f;display:grid;place-items:center;color:var(--orange2);font-size:20px}.tree-title{font-size:18px;font-weight:840;letter-spacing:-.025em;overflow-wrap:anywhere}.tree-sub{font-size:12px;color:var(--muted);margin-top:2px}.tree-meta{display:flex;gap:6px;flex-wrap:wrap;justify-content:flex-end}.device-body{display:grid;gap:10px;padding:0 16px 16px 70px}.drive-node{background:rgba(11,16,18,.7);border-color:#2a363b}.drive-summary{padding:14px 16px}.drive-id{font:800 16px/1.3 ui-monospace,SFMono-Regular,Consolas,monospace;color:#f4c18e}.segment-tree{display:grid;gap:8px;padding:0 12px 12px 46px}.segment-row{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center;border:1px solid #263238;background:#10171a;border-radius:13px;padding:12px}.segment-name{font:750 13px/1.45 ui-monospace,SFMono-Regular,Consolas,monospace;overflow-wrap:anywhere}.segment-caption{font-size:11px;color:var(--muted);margin-top:3px}.segment-actions{display:flex;gap:7px;flex-wrap:wrap}.segment-actions button{padding:8px 10px;font-size:12px}.pill{display:inline-flex;align-items:center;border:1px solid #2c3a40;background:#121a1e;border-radius:999px;color:#aab6af;padding:5px 9px;font-size:11px}.empty{padding:54px 20px;text-align:center;color:var(--muted);border:1px dashed #354248;border-radius:18px}
  ...
  @media(max-width:800px){.shell{width:min(100% - 24px,1240px);padding-top:17px}.hero-grid{grid-template-columns:1fr;gap:25px}.privacy-card{display:none}.tools-grid{grid-template-columns:1fr}.stats{grid-template-columns:1fr 1fr}.stats .stat:last-child{grid-column:1/-1}.toolbar{grid-template-columns:1fr auto auto}.toolbar #mobileLogout{display:inline-flex}.admin-head #logout{display:none}.admin-head{align-items:start}.admin-head h1{font-size:34px}.share-row{grid-template-columns:1fr}.device-body{padding-left:12px}.segment-tree{padding-left:12px}.segment-row{grid-template-columns:1fr}.segment-actions button{flex:1}.tree-meta{display:none}.drawer-panel{padding:16px}.summary-grid{grid-template-columns:1fr 1fr}.summary-item:last-child{grid-column:1/-1}.topbar{margin-bottom:20px}}
...
<div class="toolbar"><div class="search-wrap"><input id="search" type="search" placeholder="차량, 디바이스, 주행 ID 검색"></div><button id="refresh" class="secondary">새로고침</button><button id="mobileLogout" class="ghost">로그아웃</button></div>
<nav class="tabs"><button class="tab active" data-tab="routes">차량별 Routes</button><button class="tab" data-tab="shares">공유 링크</button></nav><section id="routesPanel"><div id="routes" class="vault-tree"></div></section><section id="sharesPanel" class="hidden"><div id="shares" class="share-list"></div></section><div id="status" class="tiny muted" style="margin-top:14px"></div></section>
</main>
...
<section id="shareModal" class="modal hidden"><div class="card modal-card"><div class="eyebrow">Share & analyze</div><h2>Route 공유 · 분석 도구 연결</h2><p id="shareTarget" class="muted"></p><label>링크 만료<select id="expires"><option value="7">7일</option><option value="30" selected>30일</option><option value="90">90일</option><option value="0">만료 없음</option></select></label><div class="notice" style="margin-top:15px">선택한 세그먼트가 속한 전체 route를 공유합니다. 링크를 아는 사람은 이 route의 로그·영상과 Cabana/PlotJuggler/JotPluggler API만 사용할 수 있습니다.</div><div class="modal-actions"><button id="cancelShare" class="ghost">취소</button><button id="createShare">공유·분석 링크 만들기</button></div></div></section>
<section id="shareResult" class="modal hidden"><div class="card modal-card"><div class="eyebrow">Link ready</div><h2>공유·분석 링크가 준비됐습니다</h2><p class="muted">공유 화면은 브라우저 재생용이며, 아래 API_HOST와 명령은 기존 Cabana/PlotJuggler/JotPluggler를 수정하지 않고 연결합니다.</p><label>공유 URL<div id="shareUrl" class="command share-url"><span id="shareUrlText"></span><button class="copy" data-copy-target="shareUrlText" aria-label="복사">⧉</button></div></label><label>분석 API_HOST<div class="command"><span id="apiHostText"></span><button class="copy" data-copy-target="apiHostText" aria-label="복사">⧉</button></div></label><div id="toolCommands"></div><div class="modal-actions"><a id="shareLink" class="button secondary" target="_blank" rel="noreferrer">공유 화면 열기</a><button id="closeShare">완료</button></div></div></section>
<script>
...
const packSegment=(d,r,i)=>encodeURIComponent(JSON.stringify([d,r,i]));
...
function renderRoutes(){const query=$('#search').value.trim().toLowerCase();const found=routesData.filter(r=>`${r.route} ${r.driveId||''} ${r.directory} ${r.deviceId} ${(r.segments||[]).map(s=>s.name).join(' ')}`.toLowerCase().includes(query));const devices=new Map();for(const route of found){if(!devices.has(route.directory))devices.set(route.directory,[]);devices.get(route.directory).push(route)}$('#routes').innerHTML=[...devices].map(([directory,deviceRoutes],deviceIndex)=>{const drives=new Map();for(const route of deviceRoutes){const id=route.driveId||route.route.split('--')[0];if(!drives.has(id))drives.set(id,[]);drives.get(id).push(route)}const segmentTotal=deviceRoutes.reduce((n,r)=>n+r.segmentCount,0);return `<details class="card device-node" ${query||deviceIndex===0?'open':''}><summary class="device-summary"><span class="device-icon">◆</span><div class="grow"><div class="tree-title">${esc(directory)}</div><div class="tree-sub">${deviceRoutes.length} routes · ${segmentTotal} logs</div></div><div class="tree-meta"><span class="pill">${drives.size} 주행 ID</span><span class="pill">${esc(deviceRoutes[0]?.deviceId||'device ID 없음')}</span></div></summary><div class="device-body">${[...drives].map(([driveId,driveRoutes],driveIndex)=>{const segments=driveRoutes.flatMap(route=>(route.segments||[]).map(segment=>({...segment,route}))).sort((a,b)=>a.name.localeCompare(b.name,undefined,{numeric:true}));return `<details class="soft-card drive-node" ${query||driveIndex===0?'open':''}><summary class="drive-summary"><div class="grow"><div class="drive-id">${esc(driveId)}</div><div class="tree-sub">${driveRoutes.length} route · ${segments.length} logs</div></div>${driveRoutes.some(r=>r.toolCompatible)?'<span class="pill">Cabana · PlotJuggler</span>':''}</summary><div class="segment-tree">${segments.map(item=>`<article class="segment-row"><div><div class="segment-name">${esc(item.name)}</div><div class="segment-caption">segment ${item.index} · route ${esc(item.route.route)}</div></div><div class="segment-actions"><button class="secondary" data-segment="${packSegment(item.route.directory,item.route.route,item.index)}">▶ 재생·파일</button><button data-share="${pack(item.route.directory,item.route.route)}">공유·분석</button></div></article>`).join('')}</div></details>`}).join('')}</div></details>`}).join('')||'<div class="empty">검색 조건에 맞는 로그가 없습니다.</div>'}
...
async function detail(d,r,index){const data=await api(`/api/admin/route?directory=${encodeURIComponent(d)}&route=${encodeURIComponent(r)}`);const visible=index==null?data.segments:data.segments.filter(s=>s.index===Number(index));const selected=visible[0];$('#detail').classList.remove('hidden');$('#detailTitle').textContent=selected?.name||routeDate(r);$('#detailMeta').textContent=`${d} · route ${r} · ${selected?selected.files.length:data.segmentCount} files · ${data.totalSizeLabel}`;$('#video').removeAttribute('src');$('#video').removeAttribute('poster');$('#video').load();$('#videoEmpty').classList.remove('hidden');$('#segments').innerHTML=visible.map((s,i)=>`<details class="soft-card segment" open><summary><strong class="grow">${esc(s.name)}</strong><span class="pill">${s.files.length} files</span>${s.videoUrl?`<button class="secondary" data-play="${esc(s.videoUrl)}" data-preview="${esc(s.previewUrl||'')}">▶ 재생</button>`:''}</summary><div class="segment-body"><div class="files">${s.files.map(f=>`<a class="file" href="${esc(f.url)}" target="_blank" rel="noreferrer"><span>${esc(f.name)}</span><span class="file-size">${esc(f.sizeLabel)}</span></a>`).join('')}</div></div></details>`).join('');if(selected?.videoUrl){$('#video').poster=selected.previewUrl||'';$('#video').src=selected.videoUrl;$('#videoEmpty').classList.add('hidden')}}
function openShare(d,r){shareTarget=[d,r];const route=routesData.find(x=>x.directory===d&&x.route===r);$('#shareTarget').textContent=`${d} → ${route?.driveId||r.split('--')[0]} → ${r} 전체 ${route?.segmentCount||''}개 로그`;$('#shareModal').classList.remove('hidden')}
async function createShare(){if(!shareTarget)return;const button=$('#createShare');setBusy(button,true);try{const [directory,route]=shareTarget;const data=await api('/api/admin/shares',{method:'POST',body:JSON.stringify({directory,route,expiresDays:Number($('#expires').value)})});$('#shareModal').classList.add('hidden');$('#shareResult').classList.remove('hidden');$('#shareLink').href=data.shareUrl;$('#shareUrlText').textContent=data.shareUrl;$('#apiHostText').textContent=data.apiHost;const commands=[['Cabana',data.cabanaCommand],['PlotJuggler',data.plotJugglerCommand],['JotPluggler',data.jotPlugglerCommand]].filter(x=>x[1]);$('#toolCommands').innerHTML=commands.map(([name,command],i)=>`<label>${name} 실행 명령<div id="command${i}" class="command">${esc(command)}<button class="copy" data-copy-target="command${i}" aria-label="복사">⧉</button></div></label>`).join('');await loadShares()}finally{setBusy(button,false)}}
...
$('#routes').addEventListener('click',e=>{const segment=e.target.closest('[data-segment]'),share=e.target.closest('[data-share]');if(segment)detail(...JSON.parse(decodeURIComponent(segment.dataset.segment))).catch(showError);if(share)openShare(...JSON.parse(decodeURIComponent(share.dataset.share)))});
$('#segments').addEventListener('click',e=>{const b=e.target.closest('[data-play]');if(b){e.preventDefault();$('#video').poster=b.dataset.preview||'';$('#video').src=b.dataset.play;$('#videoEmpty').classList.add('hidden');$('#video').play().catch(()=>{})}});
...
<title>Shared Carrot Route · __ROUTE_TITLE__</title><style>__COMMON_STYLE__
  .video-timeline{display:grid;grid-template-columns:44px minmax(0,1fr) 44px;align-items:center;gap:10px;padding-top:12px}.video-time{color:var(--muted);font-size:12px;font-variant-numeric:tabular-nums;text-align:center}.video-seek{width:100%;height:24px;padding:0;border:0;background:transparent;box-shadow:none;accent-color:var(--orange);cursor:pointer}.video-seek:focus{border-color:transparent;box-shadow:none}.video-seek:disabled{cursor:not-allowed;opacity:.45}
...
<section class="card card-pad"><div class="video-stage" style="margin:0"><span id="videoEmpty" class="video-empty">미리보기를 준비하는 중…</span><video id="video" controls autoplay muted playsinline preload="metadata"></video></div><div id="videoTimeline" class="video-timeline hidden"><span id="videoCurrent" class="video-time">0:00</span><input id="videoSeek" class="video-seek" type="range" min="0" max="0" value="0" step="0.01" aria-label="영상 재생 위치" disabled><span id="videoDuration" class="video-time">0:00</span></div></section>
...
<script type="module">
import { attachRadarReview } from "/assets/radar_view.js";
const $=s=>document.querySelector(s);const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const fileKind=n=>n.includes('camera')?'VIDEO':n.includes('log')?'LOG':'FILE';
const video=$('#video'),videoSeek=$('#videoSeek'),videoTimeline=$('#videoTimeline');
const radarReview=attachRadarReview(video);let radarSegments=[];
const formatTime=value=>{const seconds=Number.isFinite(value)&&value>0?Math.floor(value):0;const hours=Math.floor(seconds/3600);const minutes=Math.floor((seconds%3600)/60);const rest=seconds%60;return hours?`${hours}:${String(minutes).padStart(2,'0')}:${String(rest).padStart(2,'0')}`:`${minutes}:${String(rest).padStart(2,'0')}`};
function syncTimeline(){const duration=Number.isFinite(video.duration)&&video.duration>0?video.duration:0;const current=Number.isFinite(video.currentTime)?Math.min(video.currentTime,duration||video.currentTime):0;videoSeek.max=String(duration);videoSeek.value=String(current);videoSeek.disabled=!duration;$('#videoCurrent').textContent=formatTime(current);$('#videoDuration').textContent=formatTime(duration)}
function setVideo(source,preview,index){const segment=radarSegments.find(s=>String(s.index)===String(index));if(segment)radarReview.load(segment);video.poster=preview||'';video.src=source;video.muted=true;videoTimeline.classList.remove('hidden');$('#videoEmpty').classList.add('hidden');syncTimeline();video.play().catch(()=>{})}
['loadedmetadata','durationchange','timeupdate','seeking','seeked','emptied'].forEach(event=>video.addEventListener(event,syncTimeline));
videoSeek.addEventListener('input',()=>{const target=Number(videoSeek.value);if(Number.isFinite(target)&&Number.isFinite(video.duration)){video.currentTime=Math.min(Math.max(target,0),video.duration);syncTimeline()}});
...
async function load(){try{const r=await fetch(location.pathname+'/manifest');if(!r.ok)throw new Error(r.status===404?'공개 로그를 찾을 수 없거나 파일이 삭제되었습니다.':`HTTP ${r.status}`);const d=await r.json();radarSegments=d.segments;$('#title').textContent=d.route;$('#meta').textContent=`${d.deviceId} · 링크에 포함된 ${d.segmentCount}개 로그만 공개`;$('#summary').classList.remove('hidden');$('#summary').innerHTML=`<div class="card summary-item"><strong>${d.segmentCount}</strong><span>LOG FOLDERS</span></div><div class="card summary-item"><strong>${esc(d.totalSizeLabel)}</strong><span>ROUTE SIZE</span></div><div class="card summary-item"><strong>${d.firstSegment}–${d.lastSegment}</strong><span>SEGMENT RANGE</span></div>`;const commands=[['Cabana','CAN 메시지를 탐색하고 신호를 분석합니다.',d.cabanaCommand],['PlotJuggler','시간축으로 주행 데이터를 시각화합니다.',d.plotJugglerCommand],['JotPluggler','고속 그래프와 이벤트·신호를 함께 분석합니다.',d.jotPlugglerCommand]].filter(x=>x[2]);if(commands.length){$('#tools').classList.remove('hidden');$('#commands').innerHTML=commands.map(([name,desc,command],i)=>`<article class="card tool-card"><div class="tool-name"><span class="tool-mark">${name==='Cabana'?'C':name==='JotPluggler'?'J':'P'}</span>${name}</div><p class="muted tiny" style="margin:10px 0">${desc}</p><div class="command" id="cmd${i}">${esc(command)}<button class="copy" data-command="${encodeURIComponent(command)}">복사</button></div></article>`).join('')}const totalFiles=d.segments.reduce((n,s)=>n+s.files.length,0);$('#fileCount').textContent=`${totalFiles} files`;$('#segments').innerHTML=d.segments.map((s,i)=>`<details class="soft-card segment" ${i===0?'open':''}><summary><strong class="grow">${esc(s.name)}</strong><span class="pill">${s.files.length} files</span>${s.videoUrl?`<button data-index="${s.index}" data-play="${esc(s.videoUrl)}" data-preview="${esc(s.previewUrl||'')}">▶ 영상 재생</button>`:''}<button data-radar-index="${s.index}">레이더 검증</button></summary><div class="segment-body"><div class="files">${s.files.map(f=>`<a class="file" href="${esc(f.url)}" target="_blank" rel="noreferrer"><span class="tiny muted">${fileKind(f.name)}</span><span>${esc(f.name)}</span><span class="file-size">${esc(f.sizeLabel)}</span></a>`).join('')}</div></div></details>`).join('');const featured=d.segments.find(s=>s.videoUrl);if(featured)setVideo(featured.videoUrl,featured.previewUrl,featured.index);else if(d.segments.length)radarReview.load(d.segments[0])}catch(e){$('#status').textContent=e.message;$('#status').className='card card-pad error';$('#segments').innerHTML=''}}
document.addEventListener('click',e=>{const radar=e.target.closest('[data-radar-index]');if(radar){e.preventDefault();const s=radarSegments.find(s=>String(s.index)===radar.dataset.radarIndex);if(s?.videoUrl)setVideo(s.videoUrl,s.previewUrl,s.index);else if(s){video.pause();video.removeAttribute('src');video.load();radarReview.load(s);}scrollTo({top:0,behavior:'smooth'});}const play=e.target.closest('[data-play]');if(play){e.preventDefault();setVideo(play.dataset.play,play.dataset.preview,play.dataset.index);scrollTo({top:0,behavior:'smooth'})}const button=e.target.closest('[data-command]');if(button)copy(decodeURIComponent(button.dataset.command),button)});load();
</script></body></html>""".replace("__COMMON_STYLE__", COMMON_STYLE).replace("__BRAND__", BRAND)
