...
void VideoWidget::createCameraWidget() {
  ...
  if (auto *replay = getReplay(); replay && !replay->hasFlag(REPLAY_FLAG_NO_VIPC)) {
    ...
    for (const auto &[num, segment] : replay->route().segments()) {
      if (!segment.road_cam.empty() || !segment.qcamera.empty()) streams.insert(VISION_STREAM_ROAD);
      if (replay->hasFlag(REPLAY_FLAG_DCAM) && !segment.driver_cam.empty()) streams.insert(VISION_STREAM_DRIVER);
      if (replay->hasFlag(REPLAY_FLAG_ECAM) && !segment.wide_road_cam.empty()) streams.insert(VISION_STREAM_WIDE_ROAD);
    }
