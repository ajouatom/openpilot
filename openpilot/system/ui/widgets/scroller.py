...
class _Scroller(Widget):
  ...
  def __init__(self, items: list[Widget], horizontal: bool = True, snap_items: bool = False, spacing: int = ITEM_SPACING,
    ...
    # target offset, block manual interruption, block child widget interaction
    self._scrolling_to: tuple[float | None, bool, bool] = (None, False, False)
  ...
  def scroll_to(self, pos: float, smooth: bool = False, block_interrupt: bool = False,
                block_widget_interaction: bool = False):
  ...
  def _update_state(self):
    ...
    # Cancel auto-scroll if user starts manually scrolling (unless interruption is blocked)
