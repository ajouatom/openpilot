...
CORNER_VISION_FALLBACK_SEED_PROB = 0.85
CORNER_VISION_FALLBACK_HOLD_PROB = 0.70
CORNER_VISION_FALLBACK_CONFIRMATION_S = 0.15
CORNER_VISION_FALLBACK_VISION_HOLD_S = 0.75
CORNER_VISION_FALLBACK_MAX_X_STD_M = 8.0
CORNER_VISION_FALLBACK_MIN_DREL_M = 3.0
CORNER_VISION_FALLBACK_MAX_DREL_M = 70.0
CORNER_VISION_FALLBACK_MAX_VISION_DREL_M = 75.0
CORNER_VISION_FALLBACK_MAX_ABS_VLEAD_MPS = 0.8
CORNER_VISION_FALLBACK_HELD_MAX_ABS_VLEAD_MPS = 1.2
CORNER_VISION_FALLBACK_MAX_SPEED_DELTA_MPS = 2.0
CORNER_VISION_FALLBACK_HELD_MAX_SPEED_DELTA_MPS = 3.0
CORNER_VISION_FALLBACK_MAX_VISION_DPATH_M = 1.5
CORNER_VISION_FALLBACK_MAX_CORNER_DPATH_M = 1.0
CORNER_VISION_FALLBACK_HELD_MAX_CORNER_DPATH_M = 1.2
CORNER_VISION_FALLBACK_MAX_DPATH_DELTA_M = 1.0
CORNER_VISION_FALLBACK_MAX_ABS_YAW_RATE_RAD_S = 0.05
CORNER_VISION_FALLBACK_MIN_DISTANCE_GATE_M = 5.0
CORNER_VISION_FALLBACK_MAX_DISTANCE_GATE_M = 10.0
CORNER_VISION_FALLBACK_X_STD_SIGMA = 2.0
...
class CornerVisionPrimaryFallbackMatcher:
  """Use a central slow corner return only when primary radar is absent."""
  ...
  def reset(self) -> None:
    ...
    self._last_vision_time_s: float | None = None
    self._last_probability = 0.0
    self._last_score = 0.0

  @staticmethod
  def _primary_radar_present(
    points: Sequence[RadarPointSnapshot],
  ) -> bool:
    return any(
      point.measured
      and point.source in PRIMARY_RADAR_SOURCES
      and point.d_rel > 0.2
      for point in points
    )
  ...
  def _continuous(
    ...
    return (
      VisionRadarMatcher._stationary_position_continuous(
        previous, previous_time_s, point, time_s,
      )
      and abs(point.v_lead - previous.v_lead)
      <= CORNER_VISION_FALLBACK_MAX_SPEED_DELTA_MPS
    )
  ...
  def _vision_candidates(
    ...
    if (
      vision is None
      or vision.probability < minimum_probability
      or vision.x_std > CORNER_VISION_FALLBACK_MAX_X_STD_M
      or not CORNER_VISION_FALLBACK_MIN_DREL_M
      < vision.d_rel
      < CORNER_VISION_FALLBACK_MAX_VISION_DREL_M
      or abs(yaw_rate_rad_s)
      > CORNER_VISION_FALLBACK_MAX_ABS_YAW_RATE_RAD_S
    ):
      return []
    ...
    if (
      abs(vision_d_path)
      > CORNER_VISION_FALLBACK_MAX_VISION_DPATH_M
    ):
      return []
    distance_gate = max(
      CORNER_VISION_FALLBACK_MIN_DISTANCE_GATE_M,
      min(
        CORNER_VISION_FALLBACK_MAX_DISTANCE_GATE_M,
        vision.x_std * CORNER_VISION_FALLBACK_X_STD_SIGMA,
      ),
    )
    candidates = []
    for point in points:
      if (
        ...
        or not CORNER_VISION_FALLBACK_MIN_DREL_M
        < point.d_rel
        < CORNER_VISION_FALLBACK_MAX_DREL_M
        or abs(point.v_lead)
        > CORNER_VISION_FALLBACK_MAX_ABS_VLEAD_MPS
      ):
        continue
      ...
      speed_error = abs(point.v_lead - vision.velocity)
      d_path_error = abs(d_path - vision_d_path)
      if (
        abs(d_path) > CORNER_VISION_FALLBACK_MAX_CORNER_DPATH_M
        or d_path_error
        > CORNER_VISION_FALLBACK_MAX_DPATH_DELTA_M
      ...
      cost = (
        ...
        + speed_error / CORNER_VISION_FALLBACK_MAX_SPEED_DELTA_MPS
        + d_path_error / CORNER_VISION_FALLBACK_MAX_DPATH_DELTA_M
      )
      candidates.append((point, d_path, max(0.0, 1.0 - cost / 3.0)))
    return sorted(
      candidates,
      key=lambda candidate: (-candidate[2], candidate[0].track_id),
    )

  def _continuous_candidate(
    self,
    candidates: Sequence[tuple[RadarPointSnapshot, float, float]],
    time_s: float,
  ) -> tuple[RadarPointSnapshot, float, float] | None:
    if self._last_point is None or self._last_time_s is None:
  ...
  def _held_candidate(
    ...
    for point in points:
      if (
        ...
        and CORNER_VISION_FALLBACK_MIN_DREL_M
        < point.d_rel
        < CORNER_VISION_FALLBACK_MAX_DREL_M
        and abs(point.v_lead)
        <= CORNER_VISION_FALLBACK_HELD_MAX_ABS_VLEAD_MPS
        and self._continuous(
          self._last_point, self._last_time_s, point, time_s,
        )
      ):
        d_path = project_to_model_path(
          path, point.d_rel, point.y_rel,
        ...
        if (
          abs(d_path)
          <= CORNER_VISION_FALLBACK_HELD_MAX_CORNER_DPATH_M
        ):
          candidates.append((
            point,
            d_path,
            abs(point.d_rel - self._last_point.d_rel),
          ))
    if not candidates:
      return None
    point, d_path, _ = min(
    ...
    return point, d_path, self._last_score

  def _vision_contradicts_hold(
    self,
    vision: VisionLead | None,
    path: Sequence[tuple[float, float]],
  ) -> bool:
    if (
      vision is None
      or vision.probability < CORNER_VISION_FALLBACK_SEED_PROB
    ...
    return (
      abs(vision_d_path) > STATIONARY_FRESH_MAX_DPATH_M
      or abs(vision.d_rel - self._last_point.d_rel)
      > VISION_RADAR_MAX_DISTANCE_ERROR_M
      or abs(vision.velocity - self._last_point.v_lead)
      > CORNER_VISION_FALLBACK_HELD_MAX_SPEED_DELTA_MPS
    )

  def _match(
    self,
    candidate: tuple[RadarPointSnapshot, float, float],
    probability: float,
    time_s: float,
  ) -> VisionRadarMatch:
    point, d_path, score = candidate
  ...
  def update(
    ...
    if not math.isfinite(time_s) or len(path) < 2:
      self.reset()
      return None
    point_values = tuple(points)
    vision = vision_lead_from_model(model)
    minimum_probability = (
      CORNER_VISION_FALLBACK_HOLD_PROB
      if self._confirmed or self._pending_since_s is not None
      else CORNER_VISION_FALLBACK_SEED_PROB
    )
    ...
    candidate = self._continuous_candidate(candidates, time_s)

    if not self._confirmed:
      if self._primary_radar_present(point_values) or candidate is None:
        self.reset()
        return None
      if self._pending_since_s is None:
        assert vision is not None
        if vision.probability < CORNER_VISION_FALLBACK_SEED_PROB:
          self.reset()
          return None
        self._pending_since_s = time_s
      assert vision is not None
      self._last_vision_time_s = time_s
      match = self._match(
        candidate, vision.probability, time_s,
      )
      if (
        time_s - self._pending_since_s
        < CORNER_VISION_FALLBACK_CONFIRMATION_S
      ):
        return None
    ...
    if self._vision_contradicts_hold(vision, path):
      self.reset()
      return None
    if (
      self._last_vision_time_s is None
      or time_s - self._last_vision_time_s
      > CORNER_VISION_FALLBACK_VISION_HOLD_S
    ):
      self.reset()
      return None
    held = self._held_candidate(point_values, path, time_s)
    if held is None:
      self.reset()
      return None
    return self._match(
      held, self._last_probability, time_s,
    )
