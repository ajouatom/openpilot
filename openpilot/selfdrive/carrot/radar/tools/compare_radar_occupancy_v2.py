...
def main() -> int:
  ...
  for log_index, (path, log_entries) in enumerate(grouped.items(), 1):
    ...
    for entry in log_entries:
      ...
      rows.append(row)
      delta_text = (
        "--" if latency_delta_s is None else f"{latency_delta_s:+.2f}s"
      )
      print(
        ...
        + f"{_event_text(v2_result['event'])} delta={delta_text}",
        flush=True,
      )
  ...
  if args.report is not None:
    payload = {
      "version": 1,
      "description": "physical dPath V1 versus staged probabilistic occupancy V2",
      "rows": rows,
      "summary": {
        "v1": v1_metrics,
        "v2": v2_metrics,
  ...
  return int(missing > 0 or (args.strict_v2 and v2_failures > 0))
