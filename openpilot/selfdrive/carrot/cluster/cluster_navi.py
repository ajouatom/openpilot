...
def _guidance(item: Any, now: float) -> NaviGuidanceInfo | None:
    meta = _meta(item, now)
    if meta is None:
        return None
...
def _lane(item: Any, now: float) -> NaviLaneInfo | None:
    meta = _meta(item, now)
    if meta is None:
        return None
    return NaviLaneInfo(
...
def parse_carrot_navi(data: Any, now: float | None = None) -> NaviLiveState | None:
    ...
    lane_ahead = tuple(
        lane
        for item in (_get(data, "laneAhead", ()) or ())
        if (lane := _lane(item, now)) is not None
    )
    ...
    if speed_meta is not None:
        speed = NaviSpeedInfo(
            ...
            section_average_kph=(
                max(0.0, _float(speed_item, "sectionAverageKph")) if bool(_get(speed_item, "sectionPresent")) else None
            ),
            ...
            section_remaining_time_s=(
                max(0, _int(speed_item, "sectionRemainingTimeSec")) if bool(_get(speed_item, "sectionPresent")) else None
            ),
            section_progress=(
    ...
    state = NaviLiveState(
        ...
        vehicle=vehicle,
        current=_guidance(_get(data, "guidanceCurrent"), now),
        next=_guidance(_get(data, "guidanceNext"), now),
        lane_current=_lane(_get(data, "laneCurrent"), now),
