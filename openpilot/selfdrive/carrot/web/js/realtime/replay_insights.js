...
window.CarrotReplayInsights = window.CarrotReplayInsights || (() => {
  ...
  tabEls.forEach((element, index) => {
    ...
    element.addEventListener("keydown", (event) => {
      let nextIndex = null;
      if (event.key === "ArrowLeft") nextIndex = (index - 1 + tabEls.length) % tabEls.length;
      else if (event.key === "ArrowRight") nextIndex = (index + 1) % tabEls.length;
      else if (event.key === "Home") nextIndex = 0;
      else if (event.key === "End") nextIndex = tabEls.length - 1;
      if (nextIndex == null) return;
      event.preventDefault();
      const next = tabEls[nextIndex];
      selectTab(next.dataset.replayInsightsTab, { persist: true });
      next.focus();
    });
  });
