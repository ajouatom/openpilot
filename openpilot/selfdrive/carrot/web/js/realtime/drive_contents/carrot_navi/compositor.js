...
globalThis.CarrotNaviCompositor = (() => {
  const DPR_LIMIT = 2;
  ...
  function create(options = {}) {
    ...
    const createStagingCanvas = options.createStagingCanvas || (() => {
      ...
      if (typeof globalThis.OffscreenCanvas === "function") return new globalThis.OffscreenCanvas(1, 1);
      return null;
    });
    ...
    function render() {
      ...
      const dpr = Math.min(DPR_LIMIT, Math.max(1, globalThis.devicePixelRatio || 1));
    ...
    function fresh(maxAgeMs, now = performance.now()) {
