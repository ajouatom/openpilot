...
globalThis.CarrotNaviMse = (() => {
  const DEFAULT_MIME = 'video/mp4; codecs="avc1.42E01E"';
  const MAX_QUEUE = 12;
  const KEEP_SECONDS = 5;
  const START_SEGMENTS = 2;
  const TARGET_LAG_SECONDS = 0.55;
  const HARD_RECOVERY_LAG_SECONDS = 4;

  function constructorFor(mime = DEFAULT_MIME) {
    for (const constructor of [globalThis.ManagedMediaSource, globalThis.MediaSource]) {
      if (typeof constructor !== "function") continue;
      const checker = constructor.isTypeSupported || globalThis.MediaSource?.isTypeSupported;
      try {
  ...
  function create(video, options = {}) {
    ...
    function supported(mime = DEFAULT_MIME) {
      return Boolean(constructorFor(mime));
    }
    ...
    function destroy() {
      ...
      if (mediaSourceUrl) globalThis.URL.revokeObjectURL(mediaSourceUrl);
      mediaSourceUrl = "";
    }
    ...
    function start(initializationBuffer, mimeValue = DEFAULT_MIME) {
      ...
      const Constructor = constructorFor(mime);
      if (!Constructor) {
      ...
      mediaSourceUrl = globalThis.URL.createObjectURL(source);
    ...
    function append(buffer, metadata = {}) {
      ...
      diagnostics.lastSegmentAt = performance.now();
    ...
    function markClear(reason = "cleared") {
      ...
      diagnostics.lastClearAt = performance.now();
      diagnostics.lastSegmentAt = 0;
    }
  ...
  return Object.freeze({ DEFAULT_MIME, create, supported: (mime) => Boolean(constructorFor(mime)) });
})();
