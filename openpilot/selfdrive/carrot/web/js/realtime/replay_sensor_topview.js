...
window.CarrotReplaySensorTopview = window.CarrotReplaySensorTopview || (() => {
  ...
  const SOURCE_NAMES = ["front", "scc", "corner", "corner"];

  // Shape carries source; colour never does. This app's palette is semantic
  // (primary/info/success/warning/danger), not categorical - painting SCC green
  // or a corner radar amber would read as "ok"/"warning" rather than identity.
  // So source gets its own channel and colour is spent only on meaning:
  // the lead is the one thing this panel exists to show.
  const SOURCES = Object.freeze(["lead", "vision", "front", "scc", "corner"]);
  const SOURCE_STYLE = Object.freeze({
    vision: { shape: "circle", tone: "vision", key: "replay_sensor_source_vision", label: "Camera" },
    front: { shape: "diamond", tone: "track", key: "replay_sensor_source_front", label: "Front radar" },
    scc: { shape: "square", tone: "track", key: "replay_sensor_source_scc", label: "SCC radar" },
    corner: { shape: "triangle", tone: "track", key: "replay_sensor_source_corner", label: "Corner radar" },
    // Not a sensor: a fused lead whose originating track was not matched, so its
    // source is genuinely unknown.
    fusion: { shape: "circle", tone: "lead", key: "replay_sensor_source_fusion", label: "Combined sensors" },
    // Legend-only row. Lead is a tone, not a shape - it keeps whichever shape
    // its source has - so the key shows the tone on a neutral dot.
    lead: { shape: "circle", tone: "lead", key: "replay_sensor_source_lead", label: "Lead" },
  });
  ...
  function sourceLabel(source) {
    ...
    return style ? text(style.key, style.label) : text("replay_sensor_source_unknown", "Unidentified");
  }
  ...
  function syncHeaderStatus() {
    ...
    statusEl.textContent = note ? `${range} · ${note}` : range;
    statusEl.classList.toggle("is-warn", state.trackStatus === "error");
  }
  ...
  function renderLegend() {
    if (!legendEl) return;
    legendEl.replaceChildren(...SOURCES.map((source) => {
      const style = SOURCE_STYLE[source];
      const item = document.createElement("span");
      item.className = "carrot-replay-sensor__legendItem";
      const glyph = document.createElement("i");
      glyph.className = `carrot-replay-sensor__glyph carrot-replay-sensor__glyph--${style.shape}`;
      glyph.style.setProperty("--glyph-color", toneColor(style.tone));
      glyph.setAttribute("aria-hidden", "true");
      item.append(glyph, document.createTextNode(sourceLabel(source)));
      return item;
    }));
    legendEl.setAttribute("aria-label", text("replay_sensor_sources", "Sensor sources"));
  }

  function normalizeSource(value) {
    if (typeof value === "string") {
      if (value === "scc") return "scc";
      if (value === "corner235" || value === "corner180") return "corner";
      return "front";
    }
    return SOURCE_NAMES[Math.max(0, Math.min(3, Number(value) || 0))] || "front";
  }
  ...
  rootEl?.style.setProperty("--carrot-replay-sensor-plot-min-height", `${MIN_PLOT_HEIGHT_PX}px`);
  ...
  function showTooltip(hit, temporary = false) {
    ...
    tooltipEl.style.setProperty("--glyph-color", toneColor(tone));

    const iconEl = tooltipEl.querySelector(".carrot-replay-sensor__tooltipIcon");
    iconEl.className = `carrot-replay-sensor__tooltipIcon carrot-replay-sensor__glyph carrot-replay-sensor__glyph--${style.shape}`;
    tooltipEl.querySelector(".carrot-replay-sensor__tooltipTitle").textContent = sourceLabel(target.source);

    const stateEl = tooltipEl.querySelector(".carrot-replay-sensor__tooltipState");
    stateEl.textContent = target.selected
      ? text("replay_sensor_source_lead", "Lead")
      : (target.measured
        ? text("replay_sensor_measured", "measured")
        : text("replay_sensor_unmeasured", "estimated"));
    stateEl.classList.toggle("is-lead", Boolean(target.selected));

    const rows = [];
    rows.push(...tooltipRow(text("replay_sensor_row_distance", "Distance"), `${target.dRel.toFixed(1)} m`));
    ...
    if (Number.isFinite(relative)) {
      rows.push(...tooltipRow(
        text("replay_sensor_row_speed", "Rel. speed"),
        `${relative >= 0 ? "+" : ""}${relative.toFixed(1)} m/s`,
      ));
    }
    ...
    if (Number.isFinite(lateral)) {
      ...
      rows.push(...tooltipRow(
        text("replay_sensor_row_lateral", "Side"),
        magnitude < 0.2
    ...
    if (target.trackId >= 0) {
      rows.push(...tooltipRow(text("replay_sensor_row_track", "No."), `#${Math.trunc(target.trackId)}`));
    }
    tooltipEl.querySelector(".carrot-replay-sensor__tooltipRows").replaceChildren(...rows);

    tooltipEl.hidden = false;
    tooltipEl.classList.toggle("is-below", hit.y < 128);
    const horizontalInset = Math.min(110, Math.max(8, hit.width / 2));
    tooltipEl.style.left = `${Math.max(horizontalInset, Math.min(hit.width - horizontalInset, hit.x))}px`;
    tooltipEl.style.top = `${Math.max(8, Math.min(hit.height - 8, hit.y))}px`;
