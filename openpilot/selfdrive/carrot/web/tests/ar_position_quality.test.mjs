...
test("poor horizontal accuracy degrades to model-path placement", () => {
  ...
  input.gpsLocationExternal.horizontalAccuracy = 25;
  ...
  assert.match(quality.reasons.join(" | "), /수평 오차 25.0m/);
});

test("stale Comma GPS cannot approve a fresh TMap geo anchor", () => {
  const input = qualityInput();
  input.receivedAtMonotonic.gpsLocationExternal = 8_000;
  const quality = evaluateGeoPositionQuality(input);

  assert.equal(quality.canUseGeo, false);
  assert.equal(quality.gpsAgeMs, 2_000);
  assert.match(quality.reasons.join(" | "), /Comma GPS age 2000ms/);
});
