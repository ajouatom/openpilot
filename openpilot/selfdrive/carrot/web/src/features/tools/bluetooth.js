...
const WORDS = {
  ko: {
    ...
    gestures: "더블클릭은 두 번의 뗌 간격이 350ms 이내일 때 인식합니다. 더블클릭을 지정한 키는 짧게 누르기도 350ms 기다립니다. 길게 누르기는 700ms 이상 누르고 뗄 때 한 번 실행합니다. 키보드 자동 반복은 무시합니다. 누름 시간을 보내지 않는 리모컨은 길게 누르기를 구분할 수 없습니다. 시험 중에는 모든 키에서 세 동작을 구분합니다.",
  ...
  en: {
    ...
    gestures: "Double clicks require releases within 350ms. Assigning a double click delays the short press by 350ms. Long presses fire once on release after holding for at least 700ms. Keyboard auto-repeat is ignored. Remotes that do not report hold duration cannot distinguish long presses. Test mode recognizes all three gestures on every key.",
