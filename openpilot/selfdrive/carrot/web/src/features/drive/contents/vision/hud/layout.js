 ...
 * P0 좌하단 속도판과 활성 이벤트(방향지시등)는 숨기지 않는다. */
...
export function applyHudDegradation(root, zones = {}) {
  ...
  // P4 accel/steer/fuel/DEF: 좌측·이벤트·속도판 침범 시 우측 HUD 전체 제거.
  if (conflicts("topRight", ["topLeft", "topCenter", "bottomLeft", "bottomRight"])) apply("p4");
  // 이후에도 상단이 좁으면 좌측의 낮은 우선순위부터 단계적으로 제거.
  if (conflicts("topLeft", ["topCenter", "topRight", "bottomLeft"])) apply("p3");
  if (conflicts("topLeft", ["topCenter", "topRight", "bottomLeft"])) apply("p2");
  if (conflicts("topLeft", ["topCenter", "topRight", "bottomLeft"])) apply("p1");
