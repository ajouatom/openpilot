 ...
 */
export function createDriveVisionHudRenderer(options = {}) {
  const target = options.target || globalThis;
  function $(id) { return target.document.getElementById(id); }
  ...
  function isOverlayMount() {
    const page = target.document.body?.dataset?.page || "carrot";
    return page === "carrot";
  }
  ...
  function currentLang() {
    return typeof target.LANG === "string" && target.LANG ? target.LANG : "en";
  }
  ...
  function getHudConstraints(surface) {
    ...
    const viewportWidth = Math.max(320, Math.round(vv?.width || target.innerWidth || 0));
    const viewportHeight = Math.max(320, Math.round(vv?.height || target.innerHeight || 0));
    const isPortrait = viewportHeight > viewportWidth;
    const overlayMount = isOverlayMount();
    const surfaceTarget = getSurfaceTarget();
    const rect = surfaceTarget?.getBoundingClientRect?.() || null;
  ...
  function scheduleHudProfileApply() {
    ...
    hudLayoutRaf = target.requestAnimationFrame(() => {
      hudLayoutRaf = 0;
      applyHudProfile();
    });
  }
  ...
  function bindHudLayout() {
    ...
    target.addEventListener("resize", handleLayout, { passive: true });
    target.addEventListener("orientationchange", handleLayout, { passive: true });
    target.addEventListener("carrot:pagechange", handleLayout);
    if (target.visualViewport) {
      target.visualViewport.addEventListener("resize", handleLayout, { passive: true });
      target.visualViewport.addEventListener("scroll", handleLayout, { passive: true });
    }
    if (typeof target.ResizeObserver === "function") {
      const observer = new target.ResizeObserver(handleLayout);
  ...
  function setDriveMode(name, kind) {
    ...
    const registryLabels = typeof target.DRIVE_MODES === "object" ? target.DRIVE_MODES[lang] || target.DRIVE_MODES.en : null;
  ...
  function ensureHudAuxMetricTimer() {
    ...
    hudAuxMetricTimer = target.setInterval(() => {
      const hasDisk = hasFiniteAuxMetric(hudAuxMetricState.diskPct);
      const hasVolt = hasFiniteAuxMetric(hudAuxMetricState.voltageV);
      if (hasDisk && hasVolt) {
  ...
  return DriveVisionHudRenderer;
}

const rendererInstances = new WeakMap();

export let DriveVisionHudRenderer = null;

export function getDriveVisionHudRenderer(target = globalThis) {
  let renderer = rendererInstances.get(target);
  if (!renderer) {
    renderer = createDriveVisionHudRenderer({ target });
    rendererInstances.set(target, renderer);
  }
  if (target === globalThis) DriveVisionHudRenderer = renderer;
  return renderer;
}

export function installDriveVisionHudRendererFacade(target = globalThis) {
  const renderer = getDriveVisionHudRenderer(target);
  target.DriveVisionHudRenderer = renderer;
  return renderer;
}
