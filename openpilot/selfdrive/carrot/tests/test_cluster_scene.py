...
from cluster_config import EGO_FORWARD_M, VEHICLE_LENGTH_M
...
def test_longitudinal_render_distance_is_halved_without_changing_lateral_data() -> None:
  ...
  assert render_scene_forward_m(vehicle.longitudinal_m) == pytest.approx(EGO_FORWARD_M + 20.0)
  assert render_scene_forward_m(-20.0) == pytest.approx(EGO_FORWARD_M - 10.0)
  assert detected_vehicle_scene_forward_m(vehicle) == pytest.approx(
    EGO_FORWARD_M + 20.0 + VEHICLE_LENGTH_M * 0.5
  )
  scene = build_cluster_scene(_cluster_state(detected_vehicles=(vehicle,)))
