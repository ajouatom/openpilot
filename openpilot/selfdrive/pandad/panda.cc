...
bool Panda::up_to_date() {
  if (auto fw_sig = get_firmware_version()) {
    for (auto obj_path : { "../../../panda/board/obj/", "../../panda/board/obj/" }) {
      for (auto fn : { "panda.bin.signed", "panda_h7.bin.signed" }) {
        auto content = util::read_file(std::string(obj_path) + fn);
...
void Panda::can_send(const capnp::List<cereal::CanData>::Reader &can_data_list) {
  pack_can_buffer(can_data_list, [this](uint8_t* data, size_t size) {
    ...
    // SPI writes return the response payload length (zero on success), unlike
    // libusb writes which return the transmitted byte count.
    if (sent < 0) {
