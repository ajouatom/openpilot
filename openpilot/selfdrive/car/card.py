...
class Car:
  ...
  def __init__(self, CI=None, RI=None) -> None:
    ...
    self.card_diag_report_windows = 0
  ...
  def controls_update(self, CS: car.CarState, CC: car.CarControl):
    ...
    if self.sm.all_alive(['carControl']):
      ...
      if self.card_diag_frames >= 100:
        self.card_diag_report_windows += 1
        severe = should_log_card_diagnostics(
          self.card_diag_loop_max_us,
          self.card_diag_process_max_us,
          self.card_diag_can_timeouts,
        )
        periodic = self.card_diag_report_windows % 5 == 0
        if severe:
          cloudlog.warning(
            f"card_sendcan_diag: can_timeouts={self.card_diag_can_timeouts}, "
        ...
        if severe or periodic:
          # A 5 second cadence makes the parser/IMM radar stage measurable in
          # rlog without turning a 100 Hz timing probe into log traffic.
          cloudlog.info(
            f"card_stage_state_diag: decode_avg_us={self.card_diag_stage_sum_us['decode'] // self.card_diag_frames}, "
        ...
        if severe:
          cloudlog.warning(
            f"card_stage_send_diag: publish_avg_us={self.card_diag_stage_sum_us['publish'] // self.card_diag_frames}, "
