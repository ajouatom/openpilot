...
class CarController(CarControllerBase):
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.carFingerprint in CANFD_CAR:
      ...
      #if self.CP.extFlags & HyundaiExtFlags.SCC_BUS2.value and self.CP.flags & HyundaiFlags.CANFD_HDA2:
      #  can_sends.append(hyundaicanfd.carrot_canfd353(self.packer, self.CAN, CC.latActive, CS.canfd353_info))

      # prevent LFA from activating on HDA2 by sending "no lane lines detected" to ADAS ECU
      if self.frame % 5 == 0 and hda2 and (not (self.CP.extFlags & HyundaiExtFlags.SCC_BUS2.value) or self.CP.extFlags & HyundaiExtFlags.ACAN_PANDA.value): # SCC_BUS2의 경우 ACAN이 bus1에 있어서 보낼수가 없음..
