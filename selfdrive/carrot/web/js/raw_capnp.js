...
window.CarrotRawCapnp = (() => {
  ...
  const HUD_SCHEMAS = {
    carState: {
      fields: {
        vEgo: { kind: "float32", offset: 0 },
        aEgo: { kind: "float32", offset: 16 },
        vEgoCluster: { kind: "float32", offset: 14 },
        vCruiseCluster: { kind: "float32", offset: 19 },
        steeringAngleDeg: { kind: "float32", offset: 7 },
        useLaneLineSpeed: { kind: "float32", offset: 68 },
        brakeLights: { kind: "bool", offset: 19 },
        gearShifter: { kind: "enum", offset: 5, values: HUD_GEAR_NAMES },
      },
    },
    controlsState: {
      fields: {
        vCruiseCluster: { kind: "float32", offset: 46 },
        activeLaneLine: { kind: "bool", offset: 67 },
        curvature: { kind: "float32", offset: 37 },
        desiredCurvature: { kind: "float32", offset: 61 },
      },
    },
    deviceState: {
      fields: {
  ...
  const OVERLAY_SCHEMAS = {
    ...
    lateralPlan: {
      fields: {
        useLaneLines: { kind: "bool", offset: 29 },
    ...
    carControl: {
      fields: {
        latActive: { kind: "bool", offset: 11 },
        longActive: { kind: "bool", offset: 12 },
        actuators: {
          kind: "struct",
          offset: 2,
          schema: {
  ...
  function decodeField(message, structRef, fieldSpec) {
    switch (fieldSpec.kind) {
      ...
      case "float32":
      case "float64":
        return readScalar(message, structRef, fieldSpec.kind, fieldSpec.offset);
      case "enum": {
  ...
  function deriveHudPayload(state) {
    ...
    return {
      ...
      diskPct: isFinite(voltageMv) ? voltageMv / 1000.0 : null,
      diskLabel: "VOLT",
      vEgo,
      vSetKph: Number(
