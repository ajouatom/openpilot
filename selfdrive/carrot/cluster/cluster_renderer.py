...
TRIANGLE_STRIP_CACHE_LIMIT = 256
...
class ClusterUiRenderer:
    def __init__(
        ...
        self._triangle_strip_cache: OrderedDict[
            tuple[int, int, int],
    ...
    def _triangle_strip_points_for(self, strip: MeshStrip, count: int) -> tuple[object, int]:
        key = (id(strip.left), id(strip.right), count)
        cached = self._triangle_strip_cache.get(key)
        if cached is not None:
            left, right, points, point_count = cached
            if left is strip.left and right is strip.right:
                self._triangle_strip_cache.move_to_end(key)
                return points, point_count
            del self._triangle_strip_cache[key]
        ...
        self._triangle_strip_cache[key] = (strip.left, strip.right, points, point_count)
        while len(self._triangle_strip_cache) > TRIANGLE_STRIP_CACHE_LIMIT:
            self._triangle_strip_cache.popitem(last=False)
        return points, point_count
