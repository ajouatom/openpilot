...
vote divided by the maximum exposed clock. If VENC still cannot be resolved,
system mode prints a one-shot `System stats VENC devfreq unavailable:` line
with the probed VIDC sysfs values. When Qualcomm `msm_vidc` debugfs exposes an
active encoder instance, system mode falls back to completed-frame cadence
(`FBD Count` delta divided by the instance `fps`) and labels it as `msm_vidc`
FPS rather than a true hardware busy counter. `3` shows a large debug graph
