...
class MapD():
  ...
  def udpate_state(self, sm):
    ...
    self._disengaging = False #not controls_state.enabled and self._op_enabled
    self._op_enabled = True #controls_state.enabled

  def update_gps(self, sm):
    sock = self._gps_sock #'gpsLocationExternal'
    if not sm.updated[sock] or not sm.valid[sock]:
      return
  ...
  def update_route(self):
    def update_proc():
      ...
      if self.way_collection is None:
        _debug('Mapd *****: Can not update route. Missing WayCollection ********')
        return
      if self.location_rad is None:
        _debug('Mapd *****: Can not update route. Missing location ********')
        return
      if self.bearing_rad is None:
        _debug('Mapd *****: Can not update route. Missing bearing ********')
        return
  ...
  def publish(self, pm, sm):
    ...
    map_data_msg.valid = sm.all_alive(service_list=[self._gps_sock]) and \
                         sm.all_valid(service_list=[self._gps_sock])
...
def mapd_thread(sm=None, pm=None):
  ...
  _gps_sock = 'gpsLocationExternal' if Params().get_bool("UbloxAvailable") else 'gpsLocation'
  mapd = MapD(_gps_sock)
  ...
  # *** setup messaging
  if sm is None:
    sm = messaging.SubMaster([_gps_sock, 'controlsState'])
  if pm is None:
