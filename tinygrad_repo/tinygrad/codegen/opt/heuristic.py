...
def hand_coded_optimizations(k:Scheduler) -> Scheduler:
  ...
  if USE_TC > 0 and (len(k.axes_of(AxisType.GROUP_REDUCE, AxisType.REDUCE)) == 1 or (TC_OPT.value >= 1)):
    # TC matching is axis-order dependent. Try every candidate like current
    # openpilot tinygrad, and discard the copied scheduler if a follow-up
    # upcast/local optimization is invalid for that candidate.
    for axis in range(3):
      tk = k.copy()
      try:
